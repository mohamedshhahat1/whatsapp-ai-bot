...
def real_adapters() -> Iterator[None]:
    ...
    Updated rather than replaced, so a channel this file does not name still
    resolves normally. That is also why every adapter this repository ships
    has to appear here as it lands: an unpinned fifth adapter is visible to
    the assertions below whether they expect it or not.
...
async def test_an_instagram_delivery_resolves_both_of_its_surfaces(
    real_adapters: None,
) -> None:
    """Instagram splits into two surfaces exactly as ``page`` does.

    This asserted a single-element result until step 3, when the comment
    adapter did not exist yet: the channel was enabled and credentialed here,
    so one element was evidence that a channel without an adapter is skipped
    rather than crashing dispatch or resolving to None. Both surfaces build
    now, in the fixed private-surface-first order dispatch relies on.
    """
    adapters = meta_inbound_adapters("instagram", settings=_settings())
    try:
        assert [a.channel for a in adapters] == [INSTAGRAM_DM, INSTAGRAM_COMMENT]
        assert isinstance(adapters[0], InstagramDMAdapter)
        assert isinstance(adapters[1], InstagramCommentAdapter)
    finally:
        for adapter in adapters:
            await adapter.aclose()


async def test_switching_instagram_comments_off_leaves_the_dm_surface_alone(
    real_adapters: None,
) -> None:
    """The Instagram half of the switch independence asserted above for page.

    Worth its own case rather than trusting symmetry: both Instagram surfaces
    read the same credential pair, so a mistake in resolving one of them shows
    up here and nowhere else.
    """
    adapters = meta_inbound_adapters(
        "instagram",
        settings=_settings(enable_instagram_comments=False),
    )
    try:
...
async def test_both_instagram_surfaces_share_one_missing_credential(
    real_adapters: None,
) -> None:
    """instagram_token falls back to the page token, so blanking both is what
    it takes to leave the Instagram surfaces unconfigured.

    Asserted because the fallback is easy to forget: blanking only the
    Instagram token leaves both surfaces perfectly usable on the page token,
    and a test that expected otherwise would look like a resolver bug.
    """
    adapters = meta_inbound_adapters(
        "instagram",
        settings=_settings(
